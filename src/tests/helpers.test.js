...
describe('transformPlanningData', () => {
  ...
  it('utilise le fallback DEFAULT_MODULES_METIERS si absent des donnees source', () => {
    // Depuis l'ajout du fallback en dur dans helpers.js, modulesMetiers
    // n'est jamais null : les modules par defaut sont retournes
    const result = transformPlanningData(FLAT_DATA)
    expect(result.modulesMetiers).not.toBeNull()
    expect(Array.isArray(result.modulesMetiers.modules)).toBe(true)
    expect(result.modulesMetiers.modules.length).toBeGreaterThan(0)
  })
})
