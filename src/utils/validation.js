...
/**
 * Valide un numero AVS suisse au format 756.XXXX.XXXX.XX.
 *
 * Verification du format uniquement (prefixe 756 + 13 chiffres).
 * Note : la validation du chiffre de controle EAN-13 n'est pas implementee ici
 * car le formulaire ne la requiert pas — la liste SharePoint fait foi.
 * L'algorithme EAN-13 consiste a alterner les poids 1 et 3 sur les 12 premiers
 * chiffres, puis a verifier que (10 - (somme % 10)) % 10 == 13e chiffre.
 *
 * @param {string} value - Numero brut saisi (points et espaces tolerees)
...
/**
 * Valide la date de naissance : pas dans le futur, age entre 15 et 99 ans.
 *
 * Calcul de l'age : difference en ms divisee par la duree moyenne d'une annee
 * (365.25 jours × 24h × 3600s × 1000ms) pour tenir compte des annees bissextiles.
 * Le plancher de 15 ans correspond au seuil minimum legal de stage en Suisse.
 * Le plafond de 99 ans sert de garde-fou contre les saisies aberrantes (ex: 1900).
 *
...
export function validateDateNaissance(value) {
  ...
  // 365.25 * 24 * 60 * 60 * 1000 = duree moyenne d'une annee en ms (bissextiles incluses)
