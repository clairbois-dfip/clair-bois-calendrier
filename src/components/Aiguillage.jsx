 ...
 *
 * Pourquoi 2 questions distinctes ?
 *  - pourQui : determinee l'identite de la personne qui soumet (le stagiaire lui-meme
 *    ou un referent/partenaire). Cela pointe vers un formulaire Forms different car les
 *    champs attendus et le destinataire interne ne sont pas les memes.
 *  - dejaInscrit : distingue un nouveau dossier d'une reinstription. Le formulaire
 *    de retour (OUI) est plus court car les donnees de base existent deja.
 *  Ces 2 reponses produisent une cle composite (ex: "stages-moi-non") qui mappe
 *  vers l'URL Forms correcte dans StagesPage/ModulesMetiers.
 *
 * Cas particulier modules :
 *  Le parcours "modules" n'a qu'un seul formulaire par profil (moi/autre),
 *  donc la Q2 (dejaInscrit) est ignoree et on appelle onResult directement
 *  depuis la Q1, sans afficher la deuxieme question.
 *
 ...
 * @param {Function} props.onBack             - Retour a la HomePage
 */
import { useState } from 'react'

export default function Aiguillage({ parcours, onResult, onBack }) {
  // null = Q1 visible, toute autre valeur = Q2 visible (la reponse a Q1 est stockee ici)
    ...
    return (
      <div className="animate-fadeIn">
        ...
        <div className="grid gap-4 sm:grid-cols-2 max-w-2xl mx-auto">
          {/* Pour moi-meme :
              - modules : saute la Q2 (pas de distinction nouveau/retour dans ce parcours)
              - stages  : affiche la Q2 (dejaInscrit) pour pointer vers le bon formulaire */}
          <button
            onClick={() => {
              if (parcours === 'modules') {
          ...
          {/* Pour quelqu'un d'autre : meme logique que "moi" mais avec pourQui='autre'.
              Ce choix oriente vers les formulaires referent/partenaire. */}
          <button
            onClick={() => {
              if (parcours === 'modules') {
