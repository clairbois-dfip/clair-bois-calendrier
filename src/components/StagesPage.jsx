...
export default function StagesPage({ onBack, onGoToFormulaire }) {
  ...
  // Le calendrier s'ouvre sur le premier mois ou des dates sont selectionnables (today + 14j)
  ...
  const handleDayClick = (date) => {
    ...
    if (!rangeStart || (rangeStart && rangeEnd)) {
      // Premier clic (ou re-clic apres plage complete) : repart d'un nouveau debut
      setRangeStart(date)
      setRangeEnd(null)
    } else {
      // Deuxieme clic : ferme la plage. Si la date est avant le debut, on inverse
      if (date < rangeStart) {
        setRangeStart(date)
