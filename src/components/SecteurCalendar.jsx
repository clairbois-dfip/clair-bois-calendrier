...
export default function SecteurCalendar({
  ...
  // Index plat des semaines avec creneaux chevauchants fusionnes.
  // Cle : "annee-numSemaine" (ex: "2026-13"), Valeur : { creneaux[], totalSlots, usedSlots, status }
  // Sert a savoir, pour chaque cellule du calendrier, si une disponibilite existe.
  const weekIndex = aggregateWeekCreneaux(secteur.weeks)

  // getCalendarDays() remplit un tableau de 28 a 35 dates couvrant tout le mois,
  // en completant le debut et la fin avec null si le mois ne commence pas lundi.
  // groupByWeeks() decoupe ce tableau en sous-tableaux de 7 jours (une ligne = une semaine).
    ...
    <div className="animate-fadeIn">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div className="p-2 md:p-4">
          ...
          {/* Lignes du calendrier : une div par semaine du mois affiche */}
          {calendarWeeks.map((week, weekIdx) => {
            // Les premiers/derniers jours du tableau peuvent etre null (padding du mois)
            // On prend le premier jour reel pour calculer le numero de semaine ISO
            const firstRealDay = week.find((d) => d !== null)
            if (!firstRealDay) return null

            const weekNum = getISOWeekNumber(firstRealDay)

            // La semaine ISO peut appartenir a l'annee precedente (janvier) ou suivante (decembre)
            // ex: le 1er janvier 2026 (jeudi) appartient a la S1 de 2026, mais le 29 dec 2025
            // (lundi) appartient a la S1 de 2026 aussi → l'annee de l'affichage != l'annee ISO
            ...
            // Chercher si des creneaux existent pour cette semaine dans le JSON
            const weekData = weekIndex[`${weekYear}-${weekNum}`]
            const hasData = !!weekData

            // Garantir exactement 7 cellules par ligne (la derniere semaine peut etre incomplete)
