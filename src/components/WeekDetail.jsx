 ...
 *
 * Flux pendingWeekContext :
 *  L'utilisateur clique "S'inscrire" sur un creneau → onInscription(creneau) est appele.
 *  Dans App.jsx, cela stocke les dates du creneau dans pendingWeekContext, puis bascule
 *  vers l'aiguillage (sans effacer le contexte). Une fois les 2 questions repondues,
 *  handleAiguillageResult detecte pendingWeekContext, saute l'etape calendrier et envoie
 *  directement l'utilisateur vers le formulaire pre-rempli avec les bonnes dates.
 *  Ce mecanisme evite de forcer l'aiguillage avant le choix de semaine, ce qui casserait
 *  le parcours naturel (voir la semaine, puis s'inscrire).
 *
 ...
 * @param {Function} props.onInscription         - Declenche l'aiguillage avec le creneau en attente
...
export default function WeekDetail({
  ...
  // Retrocompatibilite : ancien format n'a pas de tableau creneaux, on l'emballe
    ...
    <div className="animate-fadeIn">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden max-w-lg mx-auto">
        ...
        <div className="p-6">
          ...
          {/* Creneau unique : un seul bouton suffit, pas besoin d'iterer */}
          {!isMulti && (
            <>
              {available > 0 ? (
                <button
          ...
          {/* Multi-creneaux : chaque creneau a ses propres dates, places et bouton.
              onInscription(c) transmet le creneau specifique a App.jsx pour pre-remplir
              les bonnes dates dans le formulaire (via pendingWeekContext). */}
          {isMulti && (
