...
// label      : libelle complet affiche dans les cartes et la legende
// labelCourt : version abregee pour le cercle quand plusieurs places du meme
//              type coexistent dans la meme table (ex. "AFP 1", "AFP 2").
//              Sans ce champ, le cercle afficherait "AFP/CFC 1" qui depasse.
const TYPES_PLACE = {
...
function Place({ place, label }) {
  ...
  // Tailwind purge les classes non presentes litteralement dans le source.
  // Les construire dynamiquement (ex. `bg-cb-${statut}`) les ferait disparaitre
  // du bundle de production. On utilise donc un mapping explicite.
  const couleurFond =
  ...
  // Reduit la taille du texte proportionnellement a la longueur du label
  // pour eviter le debordement dans le cercle de 68px.
...
function Table({ table }) {
  ...
  // Quand un type est present plusieurs fois, on numerote les cercles
  // (ex. "AFP 1", "AFP 2") pour que la coordinatrice puisse distinguer
  // quelle place est occupee sans ambiguite. Si le type n'apparait qu'une
  // fois, on affiche le label complet sans numero.
  const typeIdx = {}
...
export default function Cartographie({ onGoHome, onLogout }) {
  ...
  useEffect(() => {
    // Le timestamp en query string force le navigateur a revalider le fichier
    // plutot que de servir une copie mise en cache. Sans ce parametre, les
    // mises a jour Power Automate pourraient rester invisibles pendant plusieurs
    // heures selon la politique de cache du navigateur.
